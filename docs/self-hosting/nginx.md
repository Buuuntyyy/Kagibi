...
**Ne fonctionne pas en mode Docker Swarm** (`docker stack deploy`) : Swarm ignore les conditions `depends_on`, donc `garage-init` et `backend` démarreraient sans ordre garanti. Ce n'est pas une limite propre à Garage — `docker-compose.yaml` utilise déjà `build:` pour `backend`/`frontend`, une directive que Swarm ne supporte pas du tout.
