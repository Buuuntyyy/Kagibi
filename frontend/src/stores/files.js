  ...
  actions: {
    ...
    async downloadFile(fileId, fileName, mimeType='application/octet-stream', preview = false) {
      ...
      try {
        ...
        console.log("Déchiffrement terminé. Taille:", decryptedBlob.size, "Type:", decryptedBlob.type);
        // Debug PDF Header
        if (finalMimeType.includes('pdf')) {
            const headerBuffer = await decryptedBlob.slice(0, 8).arrayBuffer();
            const headerText = new TextDecoder().decode(headerBuffer);
            console.log("PDF Header Check:", headerText, "Hex:", new Uint8Array(headerBuffer));
            if (!headerText.startsWith('%PDF')) {
                 console.error("ERREUR CRITIQUE: Le fichier déchiffré ne commence pas par %PDF !");
                 // Si c'est du JSON (erreur API masquée), on l'affiche
                 if (headerText.startsWith('{') || headerText.startsWith('<')) {
                     const fullText = await decryptedBlob.text();
                     console.error("Contenu suspect:", fullText.substring(0, 100));
                 }
            }
        }
